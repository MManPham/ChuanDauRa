...
import com.models.testSinhvienMonhoc;
...
public class testKetQuaHocC {
    ...
    @GET
     @Path("{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public List<testSinhvienMonhoc> find(@PathParam("id") String mssv) throws ClassNotFoundException {
        List<testSinhvienMonhoc> listSv = new ArrayList<>();
        Connection con = null;
        ResultSet rs = null;
        testSinhVienC sv_C = new testSinhVienC();
        ...
        try {
            ...
            while (rs.next()) {
                testSinhvienMonhoc newSv = new testSinhvienMonhoc(
                        rs.getString("MSSV"),
                        getNameSV(mssv),
